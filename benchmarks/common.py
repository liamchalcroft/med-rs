...
class BenchmarkConfig:
    ...
    sizes: List[Tuple[int, int, int]] = field(default_factory=lambda: [
        (64, 64, 64),
        (128, 128, 128),
        (256, 256, 256),
        (512, 512, 512),
...
class BenchmarkRunner:
    ...
    def run_benchmark(
        ...
        mean_ms, std_ms, min_ms, max_ms, median_ms = time_function(
            func,
    ...
    def print_results(self):
        ...
        for op, results in by_op.items():
            print(f"\n{op}:")
            print(f"  {'Size':<15} {'Median':>10} {'Mean':>10} {'Std':>8} {'Min':>10} {'Max':>10}")
            print(f"  {'-' * 65}")
            for r in sorted(results, key=lambda x: x.size[0]):
                size_str = f"{r.size[0]}x{r.size[1]}x{r.size[2]}"
                print(f"  {size_str:<15} {r.median_ms:>10.2f} {r.mean_ms:>10.2f} {r.std_ms:>8.2f} {r.min_ms:>10.2f} {r.max_ms:>10.2f}")
